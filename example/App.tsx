...
export default function App() {
  ...
  // High FPS for smoothness; slower motion via higher `speed` and longer sequence
            ...
            {[0, 1, 2].map((row) => (
              <View key={row} style={{ flexDirection: 'row', gap: 6, alignItems: 'center', justifyContent: 'center' }}>
                {[0, 1, 2].map((col) => {
                  ...
                  if (cell === 5) return <Matrix key={key} {...common} frames={ripple(7, 7, { length: 42, wavelength: 3.5, speed: 6 })} fps={24} loop ariaLabel="ripple" />;
